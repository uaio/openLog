...
AI 可利用此工具进行页面自查，如：分析报错原因、检查接口调用、查看存储异常等。`,
  ...
  async execute(args: {
    ...
    const logs: Array<{ timestamp: number; level: string; message: string; stack?: string }> =
      logsRes.status === 'fulfilled' && logsRes.value.ok ? await logsRes.value.json() : [];
    ...
    const requests: Array<{
      ...
      networkRes.status === 'fulfilled' && networkRes.value.ok ? await networkRes.value.json() : [];
    ...
    if (storageRes.status === 'fulfilled' && storageRes.value.ok) {
      ...
      storage = {
        ...
        cookieCount: snap.cookies ? snap.cookies.split(';').filter(Boolean).length : 0,
        ...(includeStorage
          ? {
              data: {
