...
export const setStorage = {
  name: 'set_storage',
  description:
    '在手机端设置 localStorage / sessionStorage 的值，常用于模拟不同用户状态（token、feature flag 等）。',
  inputSchema: {
    type: 'object' as const,
    properties: {
  ...
  inputSchema: {
    ...
    properties: {
      storageType: {
        ...
        description: '"local" | "session" | "all"（默认 all）',
      },
      deviceId: { type: 'string' as const, description: '设备 ID（可选）' },
    },
    required: [],
...
export const highlightElement = {
  ...
  description:
    '在手机页面上高亮指定 CSS selector 对应的元素（红色边框 + 半透明背景），持续数秒后自动还原。用于确认某个 DOM 元素的位置。',
  inputSchema: {
    type: 'object' as const,
    properties: {
