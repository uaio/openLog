...
import type { WebVital, PerformanceSample, LongTask, ResourceTiming, InteractionTiming } from '../types/index.js';
    ...
    <div style={styles.tooltip}>
      ...
      {payload.map((p: any) => (
        <div key={p.dataKey} style={{ color: p.color, fontSize: '12px' }}>
          {p.name}: <strong>{p.value}{p.dataKey === 'fps' ? ' fps' : ' MB'}</strong>
        </div>
      ))}
    </div>
  );
...
export function PerformancePanel({ deviceId }: PerformancePanelProps) {
  ...
  // Resource filter
  const resourceTypes = ['all', ...Array.from(new Set(resources.map(r => r.initiatorType))).sort()];
  const filteredResources = resourceFilter === 'all'
    ? [...resources].sort((a, b) => b.duration - a.duration).slice(0, 50)
    : resources.filter(r => r.initiatorType === resourceFilter).sort((a, b) => b.duration - a.duration).slice(0, 50);
      ...
      <div style={styles.body}>
        ...
        <section style={styles.section}>
          ...
          {(!report || report.vitals.length === 0) ? (
            <div style={styles.emptyHint}>等待设备上报 Web Vitals（需页面交互才能触发部分指标）</div>
          ) : (
            <div style={styles.vitalsGrid}>
              {report.vitals.map(v => <VitalCard key={v.name} vital={v} />)}
            </div>
          )}
        </section>
          ...
          {chartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <XAxis dataKey="time" tick={{ fill: '#888', fontSize: 11 }} interval="preserveStartEnd" />
                <YAxis domain={[0, 70]} tick={{ fill: '#888', fontSize: 11 }} width={30} />
                <Tooltip content={<CustomTooltip />} />
                <ReferenceLine y={60} stroke="#4caf5066" strokeDasharray="4 2" label={{ value: '60fps', fill: '#4caf50', fontSize: 10 }} />
                <ReferenceLine y={30} stroke="#ff980066" strokeDasharray="4 2" label={{ value: '30fps', fill: '#ff9800', fontSize: 10 }} />
                <Line type="monotone" dataKey="fps" name="FPS" stroke="#4fc3f7" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </section>
          ...
          <section style={styles.section}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="heapUsed" name="已用" stroke="#ce93d8" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="heapTotal" name="分配" stroke="#5c6bc0" strokeWidth={1} strokeDasharray="4 2" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </section>
        )}
        ...
        <section style={styles.section}>
          ...
          {longTasks.length === 0 ? (
            <div style={styles.emptyHint}>暂无 Long Task（&gt;50ms 阻塞任务，需浏览器支持 longtask API）</div>
          ) : (
            <div style={styles.tableWrap}>
              <table style={styles.table}>
                ...
                <tbody>
                  {[...longTasks].sort((a, b) => b.duration - a.duration).slice(0, 30).map((t, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={styles.td}>{t.startTime.toFixed(0)}ms</td>
                      <td style={{ ...styles.td, color: t.duration > 200 ? '#f44336' : '#ff9800', fontWeight: 600 }}>{t.duration}ms</td>
        ...
        {/* Resource Timing */}
        <section style={styles.section}>
          <div style={{ ...styles.sectionTitle, display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div style={styles.filterWrap}>
              {resourceTypes.map(t => (
                <button
                  key={t}
                  style={{ ...styles.filterBtn, ...(resourceFilter === t ? styles.filterBtnActive : {}) }}
                  ...
                  {filteredResources.map((r, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={{ ...styles.td, ...styles.urlCell }} title={r.name}>
                        {r.name.replace(/^https?:\/\/[^/]+/, '').slice(0, 60) || r.name.slice(0, 60)}
                      </td>
                      <td style={{ ...styles.td, color: '#9cdcfe', fontSize: '11px' }}>{r.initiatorType}</td>
                      <td style={{ ...styles.td, color: r.duration > 1000 ? '#f44336' : r.duration > 300 ? '#ff9800' : '#4caf50', fontWeight: 600 }}>{r.duration}ms</td>
                      <td style={{ ...styles.td, color: '#888', fontSize: '11px' }}>{formatBytes(r.transferSize)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section style={styles.section}>
            ...
            <div style={styles.tableWrap}>
              <table style={styles.table}>
                ...
                <tbody>
                  {[...interactions].reverse().slice(0, 30).map((item, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={{ ...styles.td, color: '#9cdcfe' }}>{item.type}</td>
                      <td style={{ ...styles.td, color: item.duration > 200 ? '#f44336' : item.duration > 100 ? '#ff9800' : '#4caf50', fontWeight: 600 }}>{item.duration}ms</td>
                      <td style={{ ...styles.td, color: '#888', fontSize: '11px' }}>{item.target || '—'}</td>
                      <td style={{ ...styles.td, color: '#666', fontSize: '11px' }}>{item.startTime.toFixed(0)}ms</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const styles: Record<string, CSSProperties> = {
  container: { display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#1e1e1e', overflow: 'hidden' },
  header: {
    display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0,
    padding: '10px 16px', borderBottom: '1px solid #333', backgroundColor: '#252526'
  },
  title: { margin: 0, fontSize: '14px', fontWeight: 600, color: '#ccc' },
  loadingBadge: { fontSize: '12px', color: '#888' },
  liveBadge: { display: 'flex', alignItems: 'center', gap: '5px', fontSize: '12px', color: '#4fc3f7', marginLeft: 'auto' },
  liveDot: { width: 7, height: 7, borderRadius: '50%', backgroundColor: '#4caf50', display: 'inline-block' },
  body: { flex: 1, overflow: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: '20px' },
  section: { backgroundColor: '#252526', borderRadius: '6px', padding: '12px' },
  sectionTitle: { fontSize: '13px', fontWeight: 600, color: '#bbb', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' },
  vitalsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px' },
  vitalCard: {
    backgroundColor: '#1e1e1e', borderRadius: '6px', padding: '10px 12px',
  ...
  noData: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', padding: '60px 0', color: '#555', fontSize: '13px' },
  tooltip: { backgroundColor: '#1e1e1e', border: '1px solid #444', borderRadius: 4, padding: '8px 10px' },
  tooltipTime: { fontSize: '11px', color: '#888', marginBottom: 4 },
  placeholder: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#555', gap: '12px' },
  placeholderIcon: { fontSize: '48px', opacity: 0.3 },
  placeholderText: { fontSize: '14px' },
  countBadge: { backgroundColor: '#333', color: '#888', borderRadius: '10px', padding: '1px 7px', fontSize: '11px', fontWeight: 'normal' },
  tableWrap: { overflowX: 'auto', maxHeight: '240px', overflowY: 'auto' },
  table: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' },
  th: { textAlign: 'left', padding: '6px 10px', color: '#666', fontWeight: 500, borderBottom: '1px solid #333', position: 'sticky', top: 0, backgroundColor: '#252526' },
  tr: { borderBottom: '1px solid #2a2a2a' },
  td: { padding: '5px 10px', verticalAlign: 'top', color: '#ccc' },
  urlCell: { fontFamily: 'monospace', fontSize: '11px', color: '#9cdcfe', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  filterWrap: { display: 'flex', gap: '4px', flexWrap: 'wrap', marginLeft: 'auto' },
  filterBtn: { padding: '2px 8px', fontSize: '11px', border: '1px solid #444', borderRadius: '3px', backgroundColor: 'transparent', color: '#777', cursor: 'pointer' },
