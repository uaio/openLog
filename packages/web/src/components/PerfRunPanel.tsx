  ...
  return (
    <div style={{ textAlign: 'center', padding: '16px' }}>
      <div style={{
        ...
        border: `4px solid ${color}`, flexDirection: 'column'
      }}>
...
export function PerfRunPanel({ deviceId }: PerfRunPanelProps) {
  ...
  const handleExport = useCallback(() => {
    ...
    a.href = url; a.download = `perf-run-${selected.sessionId}.json`;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      {/* Toolbar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', borderBottom: '1px solid #e0e0e0', backgroundColor: '#fafafa', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }} />
        {!running ? (
          <button onClick={handleStart} disabled={!deviceId} style={{
            padding: '5px 16px', fontSize: 13, border: '1px solid #52c41a',
            borderRadius: 4, backgroundColor: '#52c41a', color: '#fff', cursor: 'pointer'
          }}>▶ 开始跑分</button>
        ) : (
          <button onClick={handleStop} style={{
            padding: '5px 16px', fontSize: 13, border: '1px solid #ff4d4f',
            borderRadius: 4, backgroundColor: '#ff4d4f', color: '#fff', cursor: 'pointer'
          }}>⏹ 停止跑分</button>
        )}
        {selected && (
          <button onClick={handleExport} style={{
            padding: '5px 12px', fontSize: 12, border: '1px solid #1890ff',
            borderRadius: 4, backgroundColor: '#fff', color: '#1890ff', cursor: 'pointer'
          }}>📤 导出</button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        {/* History sidebar */}
        <div style={{ width: 180, borderRight: '1px solid #e0e0e0', overflowY: 'auto', flexShrink: 0 }}>
          <div style={{ padding: '8px 12px', fontSize: 11, color: '#888', borderBottom: '1px solid #f0f0f0' }}>历史记录</div>
          {sessions.length === 0 && <div style={{ padding: 12, fontSize: 12, color: '#bbb', textAlign: 'center' }}>暂无记录</div>}
          {sessions.map(s => (
            <div key={s.sessionId} onClick={() => setSelected(s)} style={{
              padding: '8px 12px', cursor: 'pointer', fontSize: 12,
              borderBottom: '1px solid #f5f5f5',
              backgroundColor: selected?.sessionId === s.sessionId ? '#e6f4ff' : '#fff',
              borderLeft: selected?.sessionId === s.sessionId ? '3px solid #1890ff' : '3px solid transparent'
            }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
          {!selected ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#999' }}>
              {running ? (
                <><div style={{ fontSize: 48 }}>⏱</div><div>正在采集性能数据...</div></>
              ) : (
                <><div style={{ fontSize: 48 }}>🏁</div><div>点击"开始跑分"采集性能数据</div></>
              )}
            </div>
          ) : (
            <div style={{ maxWidth: 700 }}>
              {/* Score card */}
              <div style={{ backgroundColor: '#fff', border: '1px solid #e0e0e0', borderRadius: 8, padding: 16, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 24 }}>
                <ScoreCircle score={selected.score.total} grade={selected.score.grade} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 16, fontWeight: 'bold', color: '#333', marginBottom: 4 }}>{selected.score.summary}</div>
                  <div style={{ fontSize: 12, color: '#888' }}>
                    持续时间: {(selected.duration / 1000).toFixed(1)}s · {new Date(selected.startTime).toLocaleString()}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 16 }}>
                {selected.score.items.map(item => <MetricCard key={item.name} item={item} />)}
              </div>

              {/* Issues */}
              {selected.score.issues.length > 0 && (
                <div style={{ backgroundColor: '#fff', border: '1px solid #e0e0e0', borderRadius: 8, padding: 16 }}>
                  <div style={{ fontWeight: 'bold', fontSize: 13, marginBottom: 10 }}>⚠️ 问题列表</div>
                  {selected.score.issues.map((issue, i) => (
                    <div key={i} style={{ padding: '6px 0', borderBottom: '1px solid #f5f5f5', fontSize: 13, color: '#595959' }}>
                      • {issue}
                    </div>
                  ))}
                </div>
