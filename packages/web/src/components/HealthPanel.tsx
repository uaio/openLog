    ...
    <div style={{ padding: 20, maxWidth: 700 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <button onClick={fetchHealth} disabled={loading} style={{
          padding: '4px 12px', fontSize: 12, border: '1px solid #1890ff',
          borderRadius: 4, backgroundColor: '#fff', color: '#1890ff', cursor: 'pointer'
        }}>{loading ? '检查中...' : '🔄 刷新'}</button>
        <label style={{ fontSize: 12, color: '#666', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
          <input type="checkbox" checked={autoRefresh} onChange={e => setAutoRefresh(e.target.checked)} />
          自动刷新 (5s)
        </label>
      </div>

      {error && <div style={{ padding: '8px 12px', marginBottom: 12, backgroundColor: '#fff2f0', border: '1px solid #ffccc7', borderRadius: 4, fontSize: 13, color: '#f44336' }}>{error}</div>}
      ...
      {data && (
        <div>
          ...
          <div style={{
            ...
            backgroundColor: '#fff', border: `2px solid ${STATUS_COLOR[data.status]}`,
            borderRadius: 8, marginBottom: 16
          }}>
            <div style={{ fontSize: 48 }}>{data.status === 'healthy' ? '✅' : data.status === 'warning' ? '⚠️' : '❌'}</div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 16 }}>
            {[
              { label: '近5分钟错误数', value: data.recentErrors, unit: '次', warn: data.recentErrors > 5 },
              { label: '长任务总耗时', value: data.longTaskDurationMs, unit: 'ms', warn: data.longTaskDurationMs > 1000 },
              { label: '内存占用', value: data.memoryMB !== null ? (data.memoryMB as number).toFixed(1) : 'N/A', unit: 'MB', warn: (data.memoryMB ?? 0) > 200 },
              { label: '大体积未压缩资源', value: data.uncompressedResources, unit: '个', warn: data.uncompressedResources > 0 },
              ...
              <div key={label} style={{
                ...
                <div style={{ fontSize: 22, fontWeight: 'bold', color: warn ? '#ff4900' : '#333' }}>
                  {value}<span style={{ fontSize: 12, fontWeight: 'normal', color: '#888', marginLeft: 4 }}>{unit}</span>
                </div>
              </div>
            ))}
          </div>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' as const }}>
                {Object.entries(data.vitalRatings).map(([name, rating]) => (
                  <div key={name} style={{
                    ...
                    fontSize: 12, color: RATING_COLOR[rating] ?? '#9e9e9e', fontWeight: 'bold'
                  }}>
                    {name}: {rating}
                  </div>
                ))}
