...
function DOMNodeView({ node, depth = 0 }: DOMNodeViewProps) {
  ...
  return (
    <div style={{ marginLeft: indent }}>
      ...
      >
        {hasChildren && (
          <span style={nodeStyles.arrow}>{expanded ? '▾' : '▸'}</span>
        )}
        {!hasChildren && <span style={nodeStyles.arrowPlaceholder} />}

        <span style={nodeStyles.tag}>&lt;{node.tag}</span>
        {node.id && <span style={nodeStyles.attrId}> id=<span style={nodeStyles.attrVal}>"{node.id}"</span></span>}
        {node.className && (
          <span style={nodeStyles.attrClass}> class=<span style={nodeStyles.attrVal}>"{node.className.slice(0, 50)}"</span></span>
        )}
        {node.attrs && Object.entries(node.attrs).map(([k, v]) => (
          <span key={k} style={nodeStyles.attr}> {k}=<span style={nodeStyles.attrVal}>"{v.slice(0, 40)}"</span></span>
        ))}
      ...
      {expanded && node.children && node.children.map((child, i) => (
        <DOMNodeView key={i} node={child} depth={depth + 1} />
      ))}
      ...
      {hasChildren && expanded && (
        <div style={nodeStyles.closeTag}>&lt;/{node.tag}&gt;</div>
      )}
    </div>
  );
}
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.tree}>
        {loading && (
          <div style={styles.loadingWrap}>⏳ 加载中...</div>
        )}
        {!loading && !snapshot && (
        ...
        {!loading && snapshot && (
          <DOMNodeView node={snapshot.dom} depth={0} />
        )}
      </div>
    </div>
  );
...
const styles: Record<string, CSSProperties> = {
  ...
  header: {
    ...
    padding: '10px 16px', borderBottom: '1px solid #333', backgroundColor: '#252526', flexShrink: 0
  },
  title: { margin: 0, fontSize: '14px', fontWeight: 600, color: '#ccc' },
  headerRight: { display: 'flex', alignItems: 'center', gap: '10px' },
  meta: { display: 'flex', alignItems: 'center', gap: '6px' },
  pageTitle: { fontSize: '12px', color: '#888', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  updateTime: { fontSize: '11px', color: '#666' },
  btn: {
    padding: '4px 10px', fontSize: '12px',
    border: '1px solid #555', borderRadius: '4px',
    backgroundColor: '#333', cursor: 'pointer', color: '#ccc'
  },
  btnDisabled: { opacity: 0.4, cursor: 'not-allowed' },
  urlBar: {
    display: 'flex', alignItems: 'center', gap: '8px',
    padding: '5px 16px', backgroundColor: '#2d2d2d',
  ...
  tree: { flex: 1, overflow: 'auto', padding: '8px 8px', fontFamily: '"SF Mono", Monaco, "Cascadia Code", Consolas, monospace', fontSize: '12px' },
  loadingWrap: { padding: '40px', color: '#666', textAlign: 'center' },
  emptyWrap: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 20px', color: '#555', gap: '10px', fontSize: '13px' },
...
const nodeStyles: Record<string, CSSProperties> = {
  row: {
    display: 'flex', alignItems: 'baseline', padding: '1px 0',
  ...
  text: { color: '#d4d4d4', fontSize: '11px', marginLeft: '4px', overflow: 'hidden', textOverflow: 'ellipsis' },
