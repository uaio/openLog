...
| **SDK + PC 面板** | 远程监控，团队协作 | `npx @openlogs/cli` 启动服务 |
| **SDK + Claude Code** | AI 辅助开发，自动验证 | 再执行 `npx @openlogs/cli init` 配置 MCP |
...
# 指定端口：npx @openlogs/cli -p 8080
# 公网/云端部署：npx @openlogs/cli --host myapp.example.com
```
...
```bash
npx @openlogs/cli          # 启动，自动打印局域网 IP 和 SDK 接入代码
npx @openlogs/cli -p 8080  # 指定端口（手机和电脑不在同一 WiFi 时按需调整）
```
...
```bash
npx @openlogs/cli init     # 自动检测并配置 Claude Code / Cursor / Windsurf
```
...
- [x] `@openlogs/types` 统一数据标准（Envelope v1）
- [x] `POST /api/ingest` 外部数据接入接口
- [x] `npx @openlogs/cli init` 一键配置 AI 工具 + Claude Code slash commands
